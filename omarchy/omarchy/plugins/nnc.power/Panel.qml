...
Panel {
  ...
  // Bar-widget tint by charge level. Falls back to the theme foreground so the
  // widget still matches the bar at healthy levels.
  readonly property color barDefaultColor: root.bar ? root.bar.barForeground : Color.foreground
  readonly property color batteryLevelColor: {
    // Matches the pre-quattro waybar style.css thresholds:
    //   #battery.critical (<=10) #ff3333, #battery.warning (<=20) #e5c07b
    if (!root.batteryPresent) return root.barDefaultColor
    var pct = root.batteryFraction * 100
    if (pct <= 10) return "#ff3333"
    if (pct <= 20) return "#e5c07b"
    return root.barDefaultColor
  }
  ...
  BarIconButton {
    ...
    Behavior on foreground { ColorAnimation { duration: 220 } }
    tooltipText: ""
